...
def clinicalTestingOnly(merged_file,with_additonal_columns):
	...
	outfile=gzip.open(with_additonal_columns,'wt')
...
def runXMLpipeLine(cli_args, genome_build_id,fasta):
	...
	if spawn.find_executable('vcf-sort') is not None:
		vcf.table_to_vcf(cli_args.output_dir+genome_build_id+os.sep+"single"+os.sep+cli_args.output_prefix+"clinvar_allele_trait_pairs.single."+genome_build_id+".tsv.gz", fasta, cli_args.output_tmp+"clinvar_allele_trait_pairs.single."+genome_build_id+".unsorted.vcf")
...
def runTSVpipeLine(cli_args, genome_build_id,fasta):
	...
	if pysam_installed:
		...
		if spawn.find_executable('vcf-sort') is not None:
			vcf.table_to_vcf_oneline(cli_args.output_dir+genome_build_id+os.sep+"single"+os.sep+cli_args.output_prefix+"clinvar_allele_trait_pairs.single.clin_path.normalized."+genome_build_id+".tsv.gz", fasta, cli_args.output_tmp+"clinvar_allele_trait_pairs.single.clin_path.normalized."+genome_build_id+".unsorted.vcf")
			...
			if spawn.find_executable('bgzip') is not None:
				os.system("bgzip -f "+cli_args.output_dir+genome_build_id+os.sep+"single"+os.sep+cli_args.output_prefix+"clinvar_allele_trait_pairs.single.clin_path.normalized."+genome_build_id+".sorted.vcf")
				if spawn.find_executable('tabix') is not None:
					os.system("tabix -p vcf -f "+cli_args.output_dir+genome_build_id+os.sep+"single"+os.sep+cli_args.output_prefix+"clinvar_allele_trait_pairs.single.clin_path.normalized."+genome_build_id+".sorted.vcf.gz")
